...
class BathroomEssentialsScreen extends StatefulWidget {
  const BathroomEssentialsScreen({super.key});

  @override
  State<BathroomEssentialsScreen> createState() =>
      _BathroomEssentialsScreenState();
}

class _BathroomEssentialsScreenState extends State<BathroomEssentialsScreen> {
  final List<Map<String, dynamic>> products = [
    {'name': 'Toilet Brush', 'price': 120.0},
    {'name': 'Soap Holder', 'price': 80.0},
    {'name': 'Mug', 'price': 200.0},
    {'name': 'Bucket', 'price': 150.0},
  ];
  ...
  Future<void> fetchItems() async {
    ...
    setState(() {
      // Only show items which are in this screen's products list
      selectedItems =
          items
              .where(
  ...
  void _editItem(Map<String, dynamic> item) {
    // Only allow editing if the product still exists in this category
    if (products.any((p) => p['name'] == item['product'])) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Product ${item['product']} is not in this category.'),
        ),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Bathroom Essentials'),
        ...
        child: Column(
          children: [
            DropdownButtonFormField<String>(
              value:
                  products.any((p) => p['name'] == selectedProduct)
                      ? selectedProduct
                      : null,
            ...
            DropdownButtonFormField<String>(
              value:
                  qualities.contains(selectedQuality) ? selectedQuality : null,
            ...
            DropdownButtonFormField<String>(
              value:
                  quantities.contains(selectedQuantity)
                      ? selectedQuantity
                      : null,
                      ...
                      selectedItems.map((item) {
                        return DataRow(
                          cells: [
                            ...
                            DataCell(
                              Text('Rs. ${item['price_per_dozen'] ?? ""}'),
                            ),
                            DataCell(
                              Text(
                                'Rs. ${double.tryParse(item['total_price']?.toString() ?? "0")?.toStringAsFixed(2) ?? "0.00"}',
                              ),
                            ),
                            DataCell(
                              Row(
              ...
              Padding(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Text(
                      'Rs. ${selectedItems.fold(0.0, (sum, item) => sum + (double.tryParse(item['total_price']?.toString() ?? "0") ?? 0)).toStringAsFixed(2)}',
